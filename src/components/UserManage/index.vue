    ...
    <div class="container" @keyup.enter="searchData">
        <Container class="filter">
            ...
            <Header style="width:100%;height:auto">
                ...
                <Form class="filter-outer" :model="form" :inline="true">
                    ...
                    <FormItem class="filter-item" label="账号" style="margin-right: 26px">
                        <Input
                                v-model="form.userCodeLike"
                                size="small"
                                placeholder="请输入账号"
                        />
                    </FormItem>
                    <FormItem class="filter-item" label="用户姓名" style="margin-right: 26px">
                        <Input
                    ...
                    <FormItem class="filter-item" label="岗位:" style="margin-right: 26px">
                        <Select
                                v-model="form.postId"
                                :clearable="true"
                                placeholder="请选择岗位"
                    ...
                    <FormItem class="filter-item" label="创建时间" style="margin-right: 26px">                        
                         <DatePicker
                            v-model="form.createDt1"
                            :clearable="true"
                            placeholder="请选择日期范围"
                            size="small"
                            type="daterange"
                            ...
                            @change="createDtChange"
                            />
                    </FormItem>
                    <FormItem class="filter-item" label="用户邮箱" style="margin-right: 26px">
                        <Input
        ...
        <Container class="table-list" direction="vertical">
            ...
            <Main style="width:100%">
                <Table tooltip-effect="dark"
                    ...
                    <TableColumn :show-overflow-tooltip="true" prop="userCode" label="账号"/>
                    <TableColumn :show-overflow-tooltip="true" prop="userName" label="用户姓名"/>
                    <TableColumn label="岗位" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ postList.length>0?postList.filter((item) => item.postId == scope.row.postId )[0].postName:'暂无' }}</span>
                        </template>
                    </TableColumn>
                    <TableColumn :show-overflow-tooltip="true" prop="createDt" label="创建时间"/>
                    <TableColumn :show-overflow-tooltip="true" prop="userMail" label="用户邮箱"/>
                    <!--
                    <TableColumn label="是否有效" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span :style="'color:'+roleIsOk.filter((item) => item.value == scope.row.v )[0].color">{{ roleIsOk.filter((item) => item.value == scope.row.v )[0].label }}</span>
                        </template>
                    </TableColumn>
                    -->
        ...
        <Dialog :visible.sync="detailDialogVisible" width="950px" :before-close='closeDialog'>
            ...
            <div class="dialog-content" style="padding:0 50px">
                <Form :disabled="dialogType==3" ref="dialogForm" :model="dialogInfo" style="width: 100%">
                    <Row :gutter="10">
                        <Col :span="12" style="padding: 0 12px">
                            <FormItem class="filter-item" label="账号:" prop="userCode"
                                      :rules="{ required: true, message: '账号不能为空', trigger: 'blur'}"
                            >
                                <Input
                                        v-model="dialogInfo.userCode"
                                        size="small"
                                        style="width: 100%"
                                        placeholder="请输入账号"
                                />
                        ...
                        <Col :span="12" style="padding: 0 12px">
                            <FormItem class="filter-item" label="用户姓名:" prop="userName"
                        ...
                        <Col :span="12" style="padding: 0 12px">
                            <FormItem class="filter-item" label="岗位:" prop="postId"
                                      :rules="{ required: true, message: '岗位不能为空', trigger: 'blur'}"
                            >
                                <Select
                                        v-model="dialogInfo.postId"
                                        :clearable="true"
                                        placeholder="请选择用户岗位"
                                        style="width: 100%;"
                                        size="small"
                        ...
                        <Col :span="12" style="padding: 0 12px" v-if='dialogType != 3'>
                            <FormItem class="filter-item" label="用户密码:" prop="userPwd"
                                      :rules="{ required: true, message: '用户密码不能为空', trigger: 'blur'}"
                            >
                                <Input
                                        v-model="dialogInfo.userPwd"
                                        type="password"
                                        size="small"
                                        style="width: 100%;"
                                        placeholder="请输入用户密码"
                        ...
                        <Col :span="12" style="padding: 0 12px">
                            <FormItem class="filter-item" label="用户邮箱:" prop="userMail"
                                      :rules="{ required: true, message: '用户邮箱不能为空', trigger: 'blur'}"
                            >
                                <Input
                                        v-model="dialogInfo.userMail"
                                        size="small"
                                        style="width: 100%;"
                                        placeholder="请输入用户邮箱"
                                />
...
<script lang="ts">
    ...
    import {GetUserList, GetRoleList, AddUser, ChangeUser,GetPostList,DeleteUsers,} from './service';
    ...
    export default class Index extends Vue {
        ...
        async addList() {
            ...
            this.$refs["dialogForm"].validate(async (valid: boolean) => {
                if (valid) {
                    ...
                    let res = await AddUser(this.dialogInfo)
                    setRes(res, () => {
                         if (res.errorCode === '0') {
                             // 操作成功的回调
        ...
        async changeList() {
            ...
            this.$refs["dialogForm"].validate(async (valid: boolean) => {
                if (valid) {
                    ...
                    if (this.dialogInfo.userPwd === this.temPassWord){
                        this.dialogInfo.userPwd = ''
                    }
                    let res = await ChangeUser(this.dialogInfo)
                    setRes(res, () => {
                        // 操作成功的回调
        ...
        async showChangeDialog(e: rowType) {
            ...
            this.temPassWord = this.dialogInfo.userPwd
            if (e.type !== 'click') {
                // 更改
        ...
        async deleteList() {
            ...
            let res = await DeleteUsers(this.selectList)
            setRes(res, () => {
                this.getList();
                this.$Message.success('删除成功!');
            })
        }
        createDtChange(s: string[]) {
           // @ts-ignore
          this.form.createDtStart = s[0]+" 00:00:00";
           // @ts-ignore
          this.form.createDtEnd = s[1]+" 23:59:59";
        }
        // 显示修改详情
        ...
        async getCodeList() {
            ...
            this.postList = (await GetPostList({})).list;
        }
        // 列表多选
